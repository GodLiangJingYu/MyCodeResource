...
PROMPT_GEN2 = (
    ...
    "\n"
    "所有代码必须满足：\n"
    "1. 语法正确，能够通过 clang --analyze 编译进入静态分析阶段（必须无 error，允许无关紧要的 note）\n"
    "2. 文件扩展名为 .cpp，会按 C++ 解析；请确保生成的代码在 C++ 下可编译\n"
    "3. 只针对 {rule_set}.{rule_id} 规则，不要引入其他规则的违规\n"
    "4. 不要生成会直接导致编译错误的不完整类型对象定义（例如 `static int a[];` 这种不完整数组定义）；\n"
    "   如果需要省略数组大小，必须提供初始化器（如 `static int a[] = {{0}};`）或确保它只是声明（如 `extern int a[];`）\n"
    "5. 每份代码只输出纯 C/C++ 代码，不要包含除标记行外的任何解释"
)
