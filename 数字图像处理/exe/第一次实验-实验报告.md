# 课堂实验一　MATLAB 数字图像基本处理（Python 实现）
...
|------|------|
| 操作系统 | Windows 10/11 |
| Python 环境 | Anaconda（dip_env，Python 3.x） |
| 核心库 | OpenCV (cv2)、NumPy、Matplotlib、Pillow (PIL) |
| 开发工具 | PyCharm |
| 输入图像 | flower.tif（`media/p008_img01.png`）、Lenna.jpg（`media/p008_img03.png`）、camema.jpg（`media/p008_img04.png`） |
| 运行方式 | 命令行执行 `python main.py` |

**要求**：
- 使用 Python 完成与 MATLAB 等效的基本图像处理操作。
- 掌握图像读取、信息获取、格式转换、灰度化与二值化等基础操作。
...
程序采用顺序执行的流水线结构，依次完成图像读取、信息获取、格式转换、灰度化和二值化五个阶段：

```
图像读取 → 基本信息获取 → 详细信息获取 → 格式压缩存储 → 灰度化 → 二值化
```

### 4.2 模块划分

- **图像读取模块**：使用 `cv2.imread()` 读取图像，`cv2.cvtColor()` 进行 BGR→RGB 转换以便正确显示。
- **信息获取模块**：使用 `os.stat()` 获取文件元数据（大小），使用 NumPy 数组属性（`shape`、`dtype`）获取图像维度和数据类型，使用 `PIL.Image` 获取格式和模式信息。
- **格式转换模块**：通过 `cv2.imwrite()` 结合 `IMWRITE_JPEG_QUALITY` 参数实现 JPEG 有损压缩，直接保存实现 BMP 无压缩存储。
- **灰度化与二值化模块**：`cv2.cvtColor()` 实现 RGB→Gray 转换，`cv2.threshold()` 实现固定阈值和 Otsu 自动阈值二值化。

### 4.3 关键设计

- **多库协同**：OpenCV 负责图像读写与处理，PIL 负责格式详情获取，NumPy 负责数组操作，Matplotlib 负责结果可视化。
- **综合结果图**：使用 2×3 子图布局将原图、压缩图、Lenna、灰度图、Otsu 二值化、固定阈值二值化六幅图整合在一张图中，便于对比分析。
...
| 操作系统 | Windows 10/11 |
| Python 环境 | Anaconda（dip_env，Python 3.x） |
| 核心库 | OpenCV (cv2)、NumPy、Matplotlib、scikit-image |
| 开发工具 | PyCharm |
| 输入图像 | electric.tif（`media/p011_img01.png`，灰度电路板图像） |
| 运行方式 | 命令行执行 `python main.py` |

**要求**：
- 使用 Python 实现图像噪声添加与空域滤波操作。
- 对比均值滤波与中值滤波对不同噪声的去除效果。
...
程序按照"读取→加噪→滤波→对比"的流水线执行：

```
原始灰度图像 → 添加噪声（高斯/椒盐，不同强度） → 均值滤波（3×3, 5×5） → 中值滤波（3×3, 5×5） → 结果对比显示
```

### 4.2 噪声与滤波的组合设计

- **噪声维度**：两种噪声类型（高斯、椒盐）× 三种强度（低、中、高），共 6 组噪声图像。
- **滤波维度**：两种滤波器（均值、中值）× 两种核大小（3×3, 5×5），共 4 组滤波结果。
- 以中等强度噪声为基准进行滤波对比，确保效果差异明显。

### 4.3 关键设计

- **数据类型转换**：噪声添加在 float64 [0,1] 空间进行（`skimage.random_noise`），滤波前转回 uint8（`to_uint8()`），确保 OpenCV 滤波函数正常工作。
- **综合对比图**：使用 2×4 子图布局，上行为均值滤波对比，下行为中值滤波对比，便于横向比较不同核大小、纵向比较不同滤波器的效果差异。
...
| 开发工具 | PyCharm |
| 输入图像 | room.tif（`media/p014_img01.png`，灰度图像） |
| 运行方式 | 命令行执行 `python main.py` |

**要求**：
- 使用 NumPy FFT 实现频域滤波，设计理想、巴特沃斯、高斯三种低通/高通滤波器。
- 将频域滤波结果与空域滤波进行对比验证。
...
程序按照"FFT→滤波器设计→频域滤波→IFFT→空域对比"的流水线执行：

```
原始灰度图像 → FFT + fftshift → 频谱显示 → 设计低通/高通掩膜 → 频域相乘 → ifftshift + IFFT → 空域滤波对比
```

### 4.2 滤波器设计策略

- **低通滤波器**：分别实现理想（矩形截断）、巴特沃斯（平滑过渡，阶数 n=2）、高斯（最平滑过渡）三种传递函数，截止频率 D₀ 统一设为 50。
- **高通滤波器**：利用 H_hp = 1 - H_lp 的关系，由低通滤波器直接导出对应的高通滤波器，避免重复设计。
- **频域与空域对比**：将频域高斯低通结果与空域 `GaussianBlur` 对比，频域高通结果与空域 Sobel 边缘检测对比，验证卷积定理。

### 4.3 关键设计

- **频谱可视化**：对频谱幅值取 `20 * log(|F| + 1)` 对数变换，增强低幅值分量的可见性。
- **滤波器掩膜复用**：同一掩膜同时用于低通滤波和高通滤波（1-掩膜），减少重复计算。
- **多子图展示**：分别绘制低通结果、高通结果、频域空域对比、综合结果四张图，覆盖所有实验要求。
...
def apply_filter(img, mask):
    ...
    fshift = np.fft.fftshift(f)
    filtered = fshift * mask
    img_back = np.clip(np.real(np.fft.ifft2(np.fft.ifftshift(filtered))), 0, 255).astype(np.uint8)
    return img_back
...
def main():
    ...
    img_sobel = np.uint8(np.clip(np.sqrt(sobelx**2 + sobely**2), 0, 255))

    # 绘制结果图1: 低通滤波结果
    fig1, axs1 = plt.subplots(2, 3, figsize=(18, 12))
    fig1.suptitle('频域低通滤波结果', fontsize=16)
    axs1[0, 0].imshow(img, cmap='gray'); axs1[0, 0].set_title('Original Image'); axs1[0, 0].axis('off')
    axs1[0, 1].imshow(magnitude_spectrum, cmap='gray'); axs1[0, 1].set_title('Magnitude Spectrum'); axs1[0, 1].axis('off')
    axs1[0, 2].imshow(ilpf_mask, cmap='gray'); axs1[0, 2].set_title(f'ILPF Mask (D0={d0})'); axs1[0, 2].axis('off')
    axs1[1, 0].imshow(img_ilpf, cmap='gray'); axs1[1, 0].set_title('ILPF Result'); axs1[1, 0].axis('off')
    axs1[1, 1].imshow(img_blpf, cmap='gray'); axs1[1, 1].set_title(f'BLPF Result (n=2)'); axs1[1, 1].axis('off')
    axs1[1, 2].imshow(img_glpf, cmap='gray'); axs1[1, 2].set_title('GLPF Result'); axs1[1, 2].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_3/results_lowpass.png', dpi=150)

    # 绘制结果图2: 高通滤波结果
    fig2, axs2 = plt.subplots(2, 3, figsize=(18, 12))
    fig2.suptitle('频域高通滤波结果', fontsize=16)
    axs2[0, 0].imshow(img, cmap='gray'); axs2[0, 0].set_title('Original Image'); axs2[0, 0].axis('off')
    axs2[0, 1].imshow(ihpf_mask, cmap='gray'); axs2[0, 1].set_title(f'IHPF Mask (D0={d0})'); axs2[0, 1].axis('off')
    axs2[0, 2].imshow(bhpf_mask, cmap='gray'); axs2[0, 2].set_title(f'BHPF Mask (n=2)'); axs2[0, 2].axis('off')
    axs2[1, 0].imshow(img_ihpf, cmap='gray'); axs2[1, 0].set_title('IHPF Result'); axs2[1, 0].axis('off')
    axs2[1, 1].imshow(img_bhpf, cmap='gray'); axs2[1, 1].set_title('BHPF Result'); axs2[1, 1].axis('off')
    axs2[1, 2].imshow(img_ghpf, cmap='gray'); axs2[1, 2].set_title('GHPF Result'); axs2[1, 2].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_3/results_highpass.png', dpi=150)

    # 绘制结果图3: 频域与空域对比
    fig3, axs3 = plt.subplots(2, 3, figsize=(18, 12))
    fig3.suptitle('频域滤波与空域滤波对比', fontsize=16)
    axs3[0, 0].imshow(img, cmap='gray'); axs3[0, 0].set_title('Original'); axs3[0, 0].axis('off')
    axs3[0, 1].imshow(img_glpf, cmap='gray'); axs3[0, 1].set_title('GLPF (Frequency Domain)'); axs3[0, 1].axis('off')
    axs3[0, 2].imshow(img_gauss_spatial, cmap='gray'); axs3[0, 2].set_title('Gaussian Blur (Spatial Domain)'); axs3[0, 2].axis('off')
    axs3[1, 0].imshow(img, cmap='gray'); axs3[1, 0].set_title('Original'); axs3[1, 0].axis('off')
    axs3[1, 1].imshow(img_ghpf, cmap='gray'); axs3[1, 1].set_title('GHPF (Frequency Domain)'); axs3[1, 1].axis('off')
    axs3[1, 2].imshow(img_sobel, cmap='gray'); axs3[1, 2].set_title('Sobel Edge (Spatial Domain)'); axs3[1, 2].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_3/results_comparison.png', dpi=150)

    # 绘制结果图4: 综合结果
    fig4, axs4 = plt.subplots(3, 4, figsize=(20, 15))
    fig4.suptitle('频域滤波综合结果', fontsize=16)
    axs4[0, 0].imshow(img, cmap='gray'); axs4[0, 0].set_title('Original'); axs4[0, 0].axis('off')
    axs4[0, 1].imshow(magnitude_spectrum, cmap='gray'); axs4[0, 1].set_title('Magnitude Spectrum'); axs4[0, 1].axis('off')
    axs4[0, 2].imshow(ilpf_mask, cmap='gray'); axs4[0, 2].set_title('ILPF Mask'); axs4[0, 2].axis('off')
    axs4[0, 3].imshow(ihpf_mask, cmap='gray'); axs4[0, 3].set_title('IHPF Mask'); axs4[0, 3].axis('off')
    axs4[1, 0].imshow(img_ilpf, cmap='gray'); axs4[1, 0].set_title('ILPF'); axs4[1, 0].axis('off')
    axs4[1, 1].imshow(img_blpf, cmap='gray'); axs4[1, 1].set_title('BLPF (n=2)'); axs4[1, 1].axis('off')
    axs4[1, 2].imshow(img_glpf, cmap='gray'); axs4[1, 2].set_title('GLPF'); axs4[1, 2].axis('off')
    axs4[1, 3].imshow(img_gauss_spatial, cmap='gray'); axs4[1, 3].set_title('Spatial Gaussian'); axs4[1, 3].axis('off')
    axs4[2, 0].imshow(img_ihpf, cmap='gray'); axs4[2, 0].set_title('IHPF'); axs4[2, 0].axis('off')
    axs4[2, 1].imshow(img_bhpf, cmap='gray'); axs4[2, 1].set_title('BHPF'); axs4[2, 1].axis('off')
    axs4[2, 2].imshow(img_ghpf, cmap='gray'); axs4[2, 2].set_title('GHPF'); axs4[2, 2].axis('off')
    axs4[2, 3].imshow(img_sobel, cmap='gray'); axs4[2, 3].set_title('Sobel Edge'); axs4[2, 3].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_3/results.png', dpi=150)
...
| 核心库 | OpenCV (cv2)、NumPy、Matplotlib |
| 开发工具 | PyCharm |
| 输入图像 | flower1.tif（`media/p008_img01.png`）、多波段图像（`media/v1_red.jpg`、`v1_green.jpg`、`v1_blue.jpg`、`infer_near.jpg`、`infer_mid.jpg`） |
| 运行方式 | 命令行执行 `python main.py` |

**要求**：
- 使用 Python 实现 RGB/HSV 色彩空间转换、通道分离与合并。
- 掌握假彩色合成与伪彩色映射方法。
...
程序按照"通道分析→直方图均衡→假彩色合成→伪彩色映射→彩色变换"的流水线执行：

```
彩色图像 → RGB/HSV 通道分离 → 直方图均衡（RGB vs HSV-V） → 多波段假彩色合成 → 灰度切片伪彩色 → 正弦函数彩色变换
```

### 4.2 模块划分

- **通道分析模块**：`cv2.split()` 分离 RGB 和 HSV 各通道，单独显示并统计均值和标准差。
- **直方图均衡模块**：分别实现 RGB 三通道独立均衡和 HSV 空间仅 V 通道均衡两种方案，对比色调保持效果。
- **假彩色合成模块**：加载近红外、中红外等多波段图像，替换 RGB 通道实现假彩色合成，支持波段缺失时的模拟回退。
- **伪彩色映射模块**：使用 Matplotlib colormap（hot/cool）将灰度值映射为彩色，实现 `imadjust` 等效的灰度切片函数进行分段映射。
- **彩色变换模块**：使用正弦函数构建灰度→RGB 的查找表映射，支持 R/G 通道互换产生不同色彩效果。

### 4.3 关键设计

- **HSV 均衡优势**：仅对 V 通道均衡可避免色调偏移，是彩色图像增强的推荐做法。
- **imadjust 等效实现**：自定义 `imadjust()` 函数实现 MATLAB 的灰度范围映射功能，支持指定输入/输出范围。
- **多波段兼容性**：假彩色合成代码检查多波段图像是否存在，不存在时使用高斯模糊和拉普拉斯边缘模拟近红外/中红外波段。
- **查找表映射**：彩色变换通过预计算 256 级查找表（LUT）再按像素值索引，避免逐像素计算，效率高。
...
def main():
    ...
    # imadjust 等效的灰度切片 + 伪彩色（指导书要求）
    cut_1 = imadjust(img_normalized, 0.0925, 0.5, 0.0925, 0.5)
    cut_2 = imadjust(img_normalized, 0.5, 1.0, 0.5, 1.0)

    img_cut1_hot = (hot_cmap(cut_1)[:, :, :3] * 255).astype(np.uint8)
    img_cut2_cool = (cool_cmap(cut_2)[:, :, :3] * 255).astype(np.uint8)
    ...
    img_transformed_swap[:, :, 0] = transform_G[img_gray]  # R 用 G 的变换
    img_transformed_swap[:, :, 1] = transform_R[img_gray]  # G 用 R 的变换
    ...
    axs1[0, 0].imshow(img_rgb); axs1[0, 0].set_title('Original RGB'); axs1[0, 0].axis('off')
    axs1[0, 1].imshow(R, cmap='Reds'); axs1[0, 1].set_title('Red Channel'); axs1[0, 1].axis('off')
    axs1[0, 2].imshow(G, cmap='Greens'); axs1[0, 2].set_title('Green Channel'); axs1[0, 2].axis('off')
    axs1[0, 3].imshow(B, cmap='Blues'); axs1[0, 3].set_title('Blue Channel'); axs1[0, 3].axis('off')
    axs1[1, 0].imshow(img_rgb_eq); axs1[1, 0].set_title('RGB Histogram Eq'); axs1[1, 0].axis('off')
    axs1[1, 1].imshow(R_eq, cmap='Reds'); axs1[1, 1].set_title('R Equalized'); axs1[1, 1].axis('off')
    axs1[1, 2].imshow(G_eq, cmap='Greens'); axs1[1, 2].set_title('G Equalized'); axs1[1, 2].axis('off')
    axs1[1, 3].imshow(B_eq, cmap='Blues'); axs1[1, 3].set_title('B Equalized'); axs1[1, 3].axis('off')
    ...
    axs1[2, 0].set_title('R Histogram'); axs1[2, 0].legend()
    ...
    axs1[2, 1].set_title('G Histogram'); axs1[2, 1].legend()
    ...
    axs1[2, 2].set_title('B Histogram'); axs1[2, 2].legend()
    axs1[2, 3].imshow(img_rgb_hsv_eq); axs1[2, 3].set_title('HSV V-Channel Eq'); axs1[2, 3].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_4/results_rgb.png', dpi=150)

    # 图2: 假彩色处理
    fig2, axs2 = plt.subplots(2, 3, figsize=(18, 12))
    fig2.suptitle('假彩色处理', fontsize=16)
    axs2[0, 0].imshow(f1 if f1 is not None else img_rgb, cmap='Reds'); axs2[0, 0].set_title('vl_red (红光通道)'); axs2[0, 0].axis('off')
    axs2[0, 1].imshow(near_infrared, cmap='gray'); axs2[0, 1].set_title('近红外 (infer_near)'); axs2[0, 1].axis('off')
    axs2[0, 2].imshow(mid_infrared, cmap='gray'); axs2[0, 2].set_title('中红外 (infer_mid)'); axs2[0, 2].axis('off')
    axs2[1, 0].imshow(true_color); axs2[1, 0].set_title('可见光真彩色'); axs2[1, 0].axis('off')
    axs2[1, 1].imshow(false_color_near); axs2[1, 1].set_title('假彩色 (近红外→R)'); axs2[1, 1].axis('off')
    axs2[1, 2].imshow(false_color_mid); axs2[1, 2].set_title('假彩色 (中红外→R)'); axs2[1, 2].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_4/results_falsecolor.png', dpi=150)

    # 图3: 伪彩色处理
    fig3, axs3 = plt.subplots(2, 4, figsize=(24, 12))
    fig3.suptitle('伪彩色处理与彩色变换', fontsize=16)
    axs3[0, 0].imshow(img_gray, cmap='gray'); axs3[0, 0].set_title('Grayscale Image'); axs3[0, 0].axis('off')
    axs3[0, 1].imshow(img_hot); axs3[0, 1].set_title('Hot Colormap (all)'); axs3[0, 1].axis('off')
    axs3[0, 2].imshow(img_cool); axs3[0, 2].set_title('Cool Colormap (all)'); axs3[0, 2].axis('off')
    axs3[0, 3].imshow(img_cut1_hot); axs3[0, 3].set_title('imadjust [0.0925,0.5]\n+ Hot Colormap'); axs3[0, 3].axis('off')
    axs3[1, 0].imshow(img_cut2_cool); axs3[1, 0].set_title('imadjust [0.5,1.0]\n+ Cool Colormap'); axs3[1, 0].axis('off')
    axs3[1, 1].imshow(img_sliced); axs3[1, 1].set_title('Gray Slicing Combined'); axs3[1, 1].axis('off')
    axs3[1, 2].imshow(img_transformed); axs3[1, 2].set_title('Sin Transform\n(R=-sin, G=-cos, B=sin)'); axs3[1, 2].axis('off')
    axs3[1, 3].imshow(img_transformed_swap); axs3[1, 3].set_title('Sin Transform (R↔G swapped)'); axs3[1, 3].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_4/results_pseudocolor.png', dpi=150)

    # 图4: 综合结果
    fig4, axs4 = plt.subplots(3, 4, figsize=(20, 15))
    fig4.suptitle('彩色图像处理综合结果', fontsize=16)
    axs4[0, 0].imshow(img_rgb); axs4[0, 0].set_title('Original'); axs4[0, 0].axis('off')
    axs4[0, 1].imshow(R, cmap='Reds'); axs4[0, 1].set_title('R Channel'); axs4[0, 1].axis('off')
    axs4[0, 2].imshow(G, cmap='Greens'); axs4[0, 2].set_title('G Channel'); axs4[0, 2].axis('off')
    axs4[0, 3].imshow(B, cmap='Blues'); axs4[0, 3].set_title('B Channel'); axs4[0, 3].axis('off')
    axs4[1, 0].imshow(img_rgb_eq); axs4[1, 0].set_title('RGB Eq'); axs4[1, 0].axis('off')
    axs4[1, 1].imshow(img_rgb_hsv_eq); axs4[1, 1].set_title('HSV V-Eq'); axs4[1, 1].axis('off')
    axs4[1, 2].imshow(false_color_near); axs4[1, 2].set_title('False Color (Near-IR)'); axs4[1, 2].axis('off')
    axs4[1, 3].imshow(false_color_mid); axs4[1, 3].set_title('False Color (Mid-IR)'); axs4[1, 3].axis('off')
    axs4[2, 0].imshow(img_cut1_hot); axs4[2, 0].set_title('imadjust+Hot'); axs4[2, 0].axis('off')
    axs4[2, 1].imshow(img_cut2_cool); axs4[2, 1].set_title('imadjust+Cool'); axs4[2, 1].axis('off')
    axs4[2, 2].imshow(img_transformed); axs4[2, 2].set_title('Sin Transform'); axs4[2, 2].axis('off')
    axs4[2, 3].imshow(img_transformed_swap); axs4[2, 3].set_title('Sin (R↔G)'); axs4[2, 3].axis('off')
    plt.tight_layout(); plt.savefig('Experiment_4/results.png', dpi=150)
