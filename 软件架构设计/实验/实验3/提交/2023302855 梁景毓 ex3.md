...
flowchart LR
    ...
    MonitoringSensor -. "《extends》" .-> RemoteSensor
    RemoteSystem -. "《extends》" .-> RemoteSensor
```

### 3. 上下文类图

上下文类图将系统视为一个整体，明确系统与外部参与者/设备/外部系统的交互边界，为后续确定内部类提供依据。
...
监控操作员 --> 应急监控系统 : 查看数据/警报\n订阅通知
远程传感器 --> 应急监控系统 : 上报监控数据\n上报警报
远程监控系统 --> 应急监控系统 : 上报监控数据\n上报警报
```
...
flowchart TB
    subgraph 客户端层
        direction TB
        OpUI["操作员工作站\nOperator UI"]
        AlarmWin["警报窗口\n[required: IAlarmService]"]
        MonWin["监控窗口\n[required: IMonitorService]"]
    ...
    subgraph 服务层
        direction TB
        AlarmSvc["警报服务\n[provided: IAlarmService]"]
        MonSvc["监控数据服务\n[provided: IMonitorService]"]
    end

    subgraph 边界构件层
        direction TB
        SensorComp["远程传感器构件\n[provided: ISensorData]"]
        MSensorComp["监控传感器构件"]
        RemoteProxy["远程监控系统代理"]
    ...
    subgraph 数据层
        direction LR
        AlarmRepo[("📁 Alarm Data\nRepository")]
        MonRepo[("📁 Monitoring Data\nRepository")]
    end

    AlarmWin -- "IAlarmService" --> AlarmSvc
    MonWin -- "IMonitorService" --> MonSvc
    SensorComp -- "IAlarmService" --> AlarmSvc
    SensorComp -- "IMonitorService" --> MonSvc

    AlarmSvc -- "read/write" --- AlarmRepo
    MonSvc -- "read/write" --- MonRepo

    AlarmSvc -. "subscribe/notify" .-> AlarmWin
    MonSvc -. "subscribe/notify" .-> MonWin
```
...
O->>AW: 请求查看警报列表
AW->>AS: getAlarms(operatorId)
AS-->>AW: alarmList
AW-->>O: 展示警报列表
O->>AW: 确认/处理某警报
AW->>AS: acknowledgeAlarm(alarmId)
AS-->>AW: updated
AW-->>O: 更新警报状态
```
...
participant MS as 监控数据服务
participant MW as 监控窗口（已订阅）

RS->>MS: reportStatus(sensorId, statusData)
MS->>MS: storeStatus(statusData)
MS->>MS: checkThreshold(statusData)
alt 超出阈值
    MS-->>MW: notifyAlert(sensorId, thresholdExceeded)
else 状态正常（有订阅）
    MS-->>MW: notifyUpdate(sensorId, currentStatus)
end
Note over MS: 阈值判断后触发不同通知\n体现订阅/通知通信模式
```
...
```mermaid
flowchart TB
    subgraph 远程站点["远程站点 (Remote Site)"]
        direction LR
        MS["[device] 监控传感器"]
        RS["[device] 远程监控系统"]
    end
    ...
    subgraph 中心服务器["中心服务器 (Central Server)"]
        direction TB
        MDS["[component] 监控数据服务"]
        AS["[component] 警报服务"]
        DB[("[database] 数据仓库")]
    end

    subgraph 操作员工作站["操作员工作站 (Operator Workstation)"]
        direction TB
        MW["[component] 监控窗口"]
        AW["[component] 警报窗口"]
    end
    ...
    MDS -. "subscribe/notify" .-> MW
    AS -. "subscribe/notify" .-> AW
